...
constexpr long long kConversionControlledMaximumActiveSteps =
    kConversionControlledMaximumSteps + 2000000;
...
void write_lammps_input(const Settings& s, const Box& box,
    ...
    const double final_xlo = -0.5 * box.lx * compression_scale;
    const double final_xhi = 0.5 * box.lx * compression_scale;
    const double final_ylo = -0.5 * box.ly * compression_scale;
    const double final_yhi = 0.5 * box.ly * compression_scale;
    const double final_zlo = -0.5 * box.lz * compression_scale;
    const double final_zhi = 0.5 * box.lz * compression_scale;
    ...
    if (controlled_conversion) {
        out << "# Start conversion-controlled curing before compression\n"
            << "fix             integrate all nvt temp 800.0 800.0 50.0\n"
        ...
        out << " units box\n"
            ...
            << "if              \"$(f_xlink[2] >= " << requested_new_bonds
            << ")\" then \"jump SELF conversion_reached_during_compression\"\n\n"
            ...
            << "unfix           conversion_halt\n"
            << "if              \"$(f_xlink[2] >= " << requested_new_bonds
            << ")\" then \"jump SELF conversion_controlled_curing_complete\"\n\n"
            << "# Continue for an extra-long upper bound, halting at the requested conversion\n"
            << "fix             conversion_halt all halt 1 v_created_new_bonds >= "
            << requested_new_bonds << " error continue\n"
            << "run             " << kConversionControlledMaximumSteps << "\n"
            << "unfix           conversion_halt\n"
            << "jump            SELF conversion_controlled_curing_complete\n\n"
            << "label           conversion_reached_during_compression\n"
            ...
            << "unfix           xlink\n"
            << "# Finish compression without creating bonds beyond the target\n"
            << "fix             compress_remaining all deform 1 x final "
            << final_xlo << ' ' << final_xhi << " y final "
            << final_ylo << ' ' << final_yhi;
        if (!film) out << " z final " << final_zlo << ' ' << final_zhi;
        out << " units box\n"
            << "run             1000000\n"
            << "unfix           compress_remaining\n"
            << "# Relax the target network at the compressed dimensions\n"
            << "run             1000000\n"
            << "unfix           integrate\n"
            << "jump            SELF conversion_controlled_curing_finished\n\n"
            << "label           conversion_controlled_curing_complete\n"
            << "print           \"Conversion-controlled curing finished with "
               "$(f_xlink[2]:%.0f) new bonds; target "
            << requested_new_bonds << " of " << maximum_new_bonds << "\"\n"
            << "variable        conversion_percent delete\n"
            ...
            << "unfix           xlink\n"
            << "unfix           integrate\n";
        out << "label           conversion_controlled_curing_finished\n";
    } else if (film) {
...
void write_info(const Settings& s, const System& sys, const Box& box,
    ...
    if (controlled_conversion) {
        out << "    \"conversion_control\": {\n"
            ...
            << "      \"crosslinking_starts_before_compression\": true,\n"
            << "      \"finish_compression_after_early_halt\": true,\n"
            << "      \"halt_check_every_steps\": 1,\n"
            << "      \"extra_long_curing_steps\": "
            << kConversionControlledMaximumSteps << ",\n"
            << "      \"maximum_bond_creation_active_steps\": "
            << kConversionControlledMaximumActiveSteps << ",\n"
    ...
    out << "    \"bond_creation_active_steps\": "
        << (controlled_conversion ?
            kConversionControlledMaximumActiveSteps : 4000000LL)
        << ",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        write_lammps_input(settings, box, files);
