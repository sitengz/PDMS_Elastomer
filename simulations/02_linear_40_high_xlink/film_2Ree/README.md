# Short-linear high-crosslinker-loading reference: 2Ree film
...
BULK_REE_ANGSTROM=42.5 \
  bash simulations/02_linear_40_high_xlink/film_2Ree/run.sh
```

The runner validates `Ree_bulk`, calculates `H`, and passes it through
`--thickness`. The generator writes `linear_40_high_xlink_film_2Ree/`.
