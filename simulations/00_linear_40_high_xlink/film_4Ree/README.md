# Short-linear high-crosslinker-loading reference: 4Ree film
...
BULK_REE_ANGSTROM=42.5 \
  bash simulations/00_linear_40_high_xlink/film_4Ree/run.sh
```

The runner validates `Ree_bulk`, calculates `H`, and passes it through
`--thickness`. The generator writes `linear_40_high_xlink_film_4Ree/`.
