...
void print_help(const char *program) {
    std::cout
        ...
        << "The default is the recommended filler self-diffusion observable: exact\n"
...
TrajectoryData read_trajectory(
    ...
    if (options.selection == Selection::kFiller &&
        ...
        throw std::runtime_error("filler selection requested for a no-filler system");
    }
...
void write_report(
    ...
    if (options.particle_mode == ParticleMode::kBeads) {
        output << "Bead MSD includes internal molecular motion. Filler molecular-COM\n"
               << "MSD is preferred for PDMS-filler translational diffusion.\n";
    }
    if (info.geometry == "film") {
